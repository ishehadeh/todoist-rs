...
use super::{Client, Collaborator, Item, Label, Project, ResourceType, Transaction, User, ID};
...
impl Cache {
    ...
    pub fn get_project<'a, P: Into<PathBuf>>(&'a self, path: P) -> Option<&'a Project> {
        let mut sorted: Vec<&Project> = self.projects.iter().map(|(_, x)| x).collect();
        ...
        for (i, proj_name) in path.into().iter().enumerate() {
            ...
            while sorted.len() != 0 {
                if sorted[0].name != proj_name.to_string_lossy()
                    || sorted[0].indent != i as u8 + 1
                    || sorted[0].item_order < order
                {
                    sorted.remove(0);
                } else {
                    break;
